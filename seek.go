...
/*
	A combination of the head, tail, and seek command cli tool written in go
*/
...
func main() {
		line := []int {};
		args := lib.ArgumentParser( os.Args )
		...
		if len( args.Payload ) != 0 {
				...
				for _, e := range args.Payload {
						...
						// coverting the given line number/s to int to base ten
						i, err := strconv.ParseInt(e, 10, 23)
						if err == nil {
								// if the arguemnt passed from the command line is a interger
									 ...
									 file = e;
										...
										if len( args.Flags ) != 0 {
												...
												if err != nil {
														...
														slice_data_stream := strings.Split(data, "\n")
